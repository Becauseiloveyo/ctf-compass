...
- 杂项与取证：文件魔数识别、strings、嵌入文件扫描、ZIP/GZIP/TAR/TGZ 递归解包、伪加密 ZIP 修复、PNG 文本块、PNG/BMP LSB、indexed PNG 调色板索引 LSB、QR/一维条码识别、GIF 注释和描述符位流、JPEG COM/XMP/APP 段、MP4 隐藏轨道和 chunk 表修复。
...
- 编码与密码：Base64/Base45/Base58/Base91/Base32、Hex、Ascii85/Z85、Base64URL/JWT payload、JWT `alg=none` CTF 变体候选、URL 编码、Quoted-Printable、UUEncode、A1Z26、NATO、Morse、Polybius、Bacon、Brainfuck/Ook、零宽/空白隐写、ROT/Caesar、Affine、Rail Fence、单字节 XOR、常见 RSA 参数弱点。
