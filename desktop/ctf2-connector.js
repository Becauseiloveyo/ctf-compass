...
function loadToken() {
  ...
  if (!fs.existsSync(targetPath) || !safeStorage.isEncryptionAvailable()) return null;
  try {
...
async function requestJson(urlPath, options = {}) {
  ...
  if (refreshedToken && options.saveRefreshedToken !== false) saveToken(refreshedToken);
  ...
  if (!response.ok) {
    const message = payload?.error?.details || payload?.error?.message || payload?.message || `CTF2 request failed (${response.status})`;
...
function normalizeImportedToken(value) {
  ...
  if (!token) throw new Error("请粘贴 CTF2 登录令牌。");
  if (token.length > 16 * 1024) throw new Error("登录令牌内容过长，请确认没有粘贴整段网页内容。");
  ...
  if (!token || /\s/.test(token)) throw new Error("登录令牌格式无效，请只粘贴 localStorage 中的 token 值。");
  return token;
}

async function importToken(value) {
  const token = normalizeImportedToken(value);
  const profile = await requestJson("/api/v1/users/profile/", { tokenOverride: token, saveRefreshedToken: false });
...
async function getStatus() {
  ...
  try {
    ...
    if (error.status !== 401 && error.status !== 403) {
      return { connected: false, cookieCount: cookies.length, error: error.message };
    }
  }
  return {
    connected: Boolean(profile),
    cookieCount: cookies.length,
    profile: profile ? { id: profile.id || profile.user_id || "", username: profile.username || profile.nickname || profile.name || "", avatar: profile.avatar || "" } : null,
  };
}
...
async function openLogin(parentWindow) {
  ...
  loginWindow = new BrowserWindow({
    ...
    webPreferences: { partition: CTF2_PARTITION, contextIsolation: true, nodeIntegration: false, sandbox: true },
  });
  loginWindow.webContents.setWindowOpenHandler(({ url }) => {
    if (url.startsWith(CTF2_ORIGIN)) return { action: "allow" };
    shell.openExternal(url);
    return { action: "deny" };
  });
  loginWindow.on("closed", () => { loginWindow = null; });
  loginWindow.on("close", () => { captureLoginToken().catch(() => {}); });
  loginWindow.webContents.on("did-navigate", () => { captureLoginToken().catch(() => {}); });
  loginWindow.webContents.on("did-navigate-in-page", () => { captureLoginToken().catch(() => {}); });
...
async function logout() {
  try {
    ...
    // Local session cleanup is authoritative even if the server session has expired.
  }
...
function flattenPracticeGrounds(payload) {
  ...
  return grounds.flatMap((ground) =>
    (Array.isArray(ground.challenges) ? ground.challenges : []).map((challenge) => ({
      ...
      files: (Array.isArray(challenge.files) ? challenge.files : []).map((file) => ({ id: file.id, name: file.file_name || "attachment.bin", size: Number(file.file_size || 0), type: file.file_type || "" })),
    })),
  );
}

async function fetchPublicChallenges(force = false) {
  if (!force && publicPracticeCache && Date.now() - publicPracticeCache.createdAt < CACHE_TTL_MS) return publicPracticeCache.challenges;
...
async function listChallenges(filters = {}) {
  ...
  const filtered = all.filter((challenge) => {
    if (ground && challenge.groundName.toLowerCase() !== ground) return false;
    if (category && category !== "all" && challenge.category.toLowerCase() !== category) return false;
    if (!query) return true;
    return [challenge.name, challenge.category, challenge.description, challenge.friendlyId].join("\n").toLowerCase().includes(query);
  });
...
async function downloadFile(challenge, file, downloadRoot) {
  ...
  const response = await connectorSession().fetch(`${CTF2_ORIGIN}${url}`, { headers: { Accept: "application/octet-stream", ...(token ? { Authorization: `Bearer ${token}` } : {}) } });
  if (!response.ok) {
  ...
  if (declaredLength > MAX_DOWNLOAD_BYTES) throw new Error(`附件超过 ${Math.round(MAX_DOWNLOAD_BYTES / 1024 / 1024)} MB 安全限制。`);
  const bytes = Buffer.from(await response.arrayBuffer());
  if (bytes.length > MAX_DOWNLOAD_BYTES) throw new Error(`附件超过 ${Math.round(MAX_DOWNLOAD_BYTES / 1024 / 1024)} MB 安全限制。`);
...
async function importChallenge(input, sandboxDownloadsRoot) {
  ...
  if (!challenge) throw new Error("没有在 CTF2 公共题库中找到该题目，请刷新题库后重试。");
  if (!challenge.files.length) throw new Error("该题目没有可下载附件；靶机题需要在 CTF2 页面中手动启动环境。");
  const status = await getStatus();
  if (!status.connected) throw new Error("附件下载需要连接 CTF2，请先使用应用内登录或浏览器令牌导入。");
  const challengeRoot = path.join(sandboxDownloadsRoot, "ctf2", `${sanitizeSegment(challenge.name)}-${sanitizeSegment(challenge.friendlyId || String(challenge.id).slice(0, 8))}`);
  const paths = [];
  for (const file of challenge.files) paths.push(await downloadFile(challenge, file, challengeRoot));
...
module.exports = { getStatus, importToken, importChallenge, listChallenges, logout, openLogin, openSystemLogin };
