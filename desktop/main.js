...
const UPDATE_RELEASE_API = "https://api.github.com/repos/Becauseiloveyo/ctf-compass/releases/latest";
...
function clearCtf2Data() {
  ...
  return { cleared: true, root, history: [], size: measureDirectory(root) };
}
...
function fetchJson(url) {
  return new Promise((resolve, reject) => {
    const request = https.get(
      url,
      {
        headers: { Accept: "application/vnd.github+json", "User-Agent": "CTF-Compass-Updater" },
        timeout: 8000,
      },
      (response) => {
        const chunks = [];
...
async function checkForUpdates() {
  ...
  return {
    ...
    url: release.html_url || "https://github.com/Becauseiloveyo/ctf-compass/releases/latest",
...
async function selectFiles() {
  const result = await dialog.showOpenDialog({ title: "Select challenge files", properties: ["openFile", "multiSelections"] });
  if (result.canceled) return [];
  return prepareArtifactsFromEntries(result.filePaths);
}

async function selectFolder() {
  const result = await dialog.showOpenDialog({ title: "Select challenge folder", properties: ["openDirectory"] });
...
ipcMain.handle("run-artifact-action", async (_event, payload) => runArtifactAction(payload?.actionId, payload?.filePath, buildRunOutputRoot()));
ipcMain.handle("reveal-artifact", async (_event, targetPath) => {
  if (targetPath) shell.showItemInFolder(targetPath);
});
ipcMain.handle("open-external", async (_event, url) => {
  const target = String(url || "");
  if (!/^https:\/\/github\.com\/Becauseiloveyo\/ctf-compass\/releases\/?/i.test(target)) {
...
ipcMain.handle("export-report", async (_event, payload) => {
  ...
  const result = await dialog.showSaveDialog({ title: "Export Markdown report", defaultPath: path.join(app.getPath("documents"), suggestedName), filters: [{ name: "Markdown", extensions: ["md"] }, { name: "Text", extensions: ["txt"] }] });
  if (result.canceled || !result.filePath) return { canceled: true };
  ensureParentDir(result.filePath);
  fs.writeFileSync(result.filePath, content, "utf8");
  return { filePath: result.filePath };
});
ipcMain.handle("app-meta", async () => ({ version: app.getVersion(), packaged: app.isPackaged, mode: isDev ? "development" : "production", sandboxRoot: sandboxRootPath() }));
