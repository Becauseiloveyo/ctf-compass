...
- Built-in analyzers handle deterministic local tasks such as recursive ZIP/GZIP/TAR/TGZ extraction, strings, encoded text layers including JWT payloads and `alg=none` CTF hints, bundled offline OCR, PNG text chunks, PNG/BMP LSB candidates including indexed-palette PNG index streams, GIF extension text, QR/barcode detection, disk/memory forensic triage, pcap triage including HTTP Basic decode, FTP control extraction, SMTP/MIME base64 attachment extraction, and TFTP DATA reassembly, PDF/Office unpacking, WAV clues, ELF checksec/Pwn surface/gadget triage, and ELF/PE/APK structure summaries.
