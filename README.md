...
- automatically extract solvable image clues such as bundled offline OCR, appended payloads, fixed-block interleaved files, PNG text chunks, PNG/BMP low-bit-plane candidates including indexed-palette PNG index LSB, GIF comment/application/plain-text extensions, GIF image-descriptor bitstreams, and JPEG COM / XMP / APP segment payloads
...
- automatically summarize local traffic captures, extracting HTTP requests, HTTP Basic credentials, DNS names, TLS SNI, cookies/tokens, FTP control commands and credentials, SMTP/MIME base64 attachments, device-separated USB HID keyboard text, USB mouse tracks, Xbox-style controller state and stick views, exported HTTP objects, directional TCP/UDP streams, TFTP DATA transfers, ICMP payload channels, DNS label streams, and IPv4 ID/TTL covert-channel candidates
...
- automatically inspect WAV metadata, PCM LSB candidates, DTMF phone-key tones, tone / morse hints, leading-alphabet tone maps, and waveform / spectrogram views for audio-based local challenges
...
- `web`: authorized local/private/public-CTF-target crawling, flexible address normalization, JWT payload decoding, `alg=none` CTF mutation hints, robots/sitemap/source-map discovery, response clue extraction, and downloaded-artifact recursion
...
- `forensic`: pcap/pcapng stream reconstruction, HTTP Basic decode, FTP control triage, SMTP/MIME attachment extraction, TFTP DATA reassembly, ICMP/DNS/IP covert-channel candidates, USB HID reconstruction, MBR/GPT and file-system identification, minidump/raw-memory indicator recovery, archive recursion, document extraction, and hidden-artifact oriented workflow hints
...
- Built-in analyzers handle deterministic local tasks such as recursive ZIP/GZIP/TAR/TGZ extraction, strings, encoded text layers including Base45, JWT payloads and `alg=none` CTF hints, bundled offline OCR, PNG text chunks, PNG/BMP LSB candidates including indexed-palette PNG index streams, GIF extension text, QR/barcode detection, disk/memory forensic triage, pcap triage including HTTP Basic decode, FTP control extraction, SMTP/MIME base64 attachment extraction, and TFTP DATA reassembly, PDF/Office unpacking, WAV DTMF/tone/LSB clues, ELF checksec/Pwn surface/gadget triage, and ELF/PE/APK structure summaries.
